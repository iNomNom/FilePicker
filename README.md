...
An easy-to-use Kotlin library for Android providing a configurable BottomSheet to pick images, videos, documents, and other files.
...
*   **Simple API:** Launch the picker with a single static method call.
*   **Modern UI:** Uses a Material Design Bottom Sheet for source selection.
*   **Highly Configurable:**
    ...
    *   Enable/disable Camera, Gallery (Photos/Videos), and File/Document sources.
*   **Image Compression:** Optionally compress images taken directly via the integrated camera source.
*   **Activity Result API:** Built on modern Android practices.
*   **Kotlin & Coroutines:** Written entirely in Kotlin, leveraging coroutines for background tasks.
*   **Permission Handling:** Handles necessary runtime permissions (like Camera) automatically when needed.
*   **Convenient Result Handling:** Uses a sealed `PickerResult` class (`Success`, `Error`, `Cancelled`).
...
2.  Add the dependency to your **app-level `build.gradle.kts`** file:
    ...
    dependencies {
        implementation("io.github.inomnom:filepicker:1.0.1")
    }
    ```
...
4.  **(Optional but Recommended)** Add the `CAMERA` permission to your app's `AndroidManifest.xml` if you intend to allow camera usage via the picker's configuration:
...
1.  **Create a `PickerConfig`** (or use a preset).
2.  **Show the Bottom Sheet:** Call `FilePickerBottomSheet.show()`, passing the `FragmentManager`, the `PickerConfig`, and a lambda to handle the result.

```kotlin
import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.util.Log
import io.github.inomnom.filepicker.api.FilePickerBottomSheet
...
// Import your binding if using ViewBinding
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Example: Pick a single image, optionally compressed
        binding.buttonPickImage.setOnClickListener {
            showPicker(PickerConfig.singleImage(compress = true))
        }

        // Example: Pick multiple documents (PDF or DOCX, max 3)
        binding.buttonPickDocs.setOnClickListener {
            ...
            showPicker(docConfig)
        }

         // Example: Pick any single file using a custom configuration
         binding.buttonPickCustom.setOnClickListener {
            val customConfig = PickerConfig.custom(
                allowedMimeTypes = setOf(MimeType.IMAGE_PNG, MimeType.APPLICATION_ZIP),
                allowMultipleSelection = true,
                maxSelectionCount = 4,
                allowCamera = true, // Show camera only if PNG is picked? (Check behavior)
                compressCameraImage = false,
                allowFiles = true,
                allowGallery = true
            )
            showPicker(customConfig)
        }
    }

    private fun showPicker(config: PickerConfig) {
        FilePickerBottomSheet.show(
            fragmentManager = supportFragmentManager,
            config = config,
            onResult = this::handlePickerResult // Method reference to the handler
        )
    }
...
```
Check [MainActivity](app/src/main/java/com/filepicker/demo/MainActivity.kt) for working examples.
## Configuration (`PickerConfig`) 🛠️

The `PickerConfig` data class allows fine-grained control over the picker's behavior.

### Presets

Convenient static functions are provided for common use cases:

| Preset Function              | Description                                        | Key Options (`MIME Types`, `Multiple?`, `Max`, `Compress?`, `Sources`)      |
| :--------------------------- | :------------------------------------------------- | :-------------------------------------------------------------------------- |
...
| `singlePdf()`                | Pick one PDF document.                             | `application/pdf`, Single, 1, `false`, Camera(No)/Gallery(No)/Files       |
| `multiplePdfs(max=5)`        | Pick up to 5 PDF documents.                        | `application/pdf`, Multiple, 5, `false`, Camera(No)/Gallery(No)/Files       |
| `singleDocument()`           | Pick one document (uses `MimeType.DOCUMENT_TYPES`).| *See `MimeType`*, Single, 1, `false`, Camera(No)/Gallery(No)/Files        |
| `multipleDocuments(max=2)`   | Pick up to 2 documents.                            | *See `MimeType`*, Multiple, 2, `false`, Camera(No)/Gallery(No)/Files        |
...
| `singleAudio()`              | Pick one audio file.                               | `audio/*`, Single, 1, `false`, Camera(No)/Gallery(No)/Files               |
| `multipleAudio(max=10)`      | Pick up to 10 audio files.                         | `audio/*`, Multiple, 10, `false`, Camera(No)/Gallery(No)/Files              |

*(Note: `Camera`/`Gallery` availability depends on whether image/video MIME types are allowed in the config.)*
...
val customConfig = PickerConfig(
    ...
    allowCamera = true,   // Show camera option (if images allowed)
    allowGallery = true,  // Show gallery option (if images/videos allowed)
    allowsFiles = true,   // Show files/documents option
...
val customConfig2 = PickerConfig.custom(
    ...
    allowCamera = false, // Explicitly disable camera even if images allowed
    allowGallery = true,
    allowFiles = false, // Disable file system browsing
    compressCameraImage = false
)

FilePickerBottomSheet.show(supportFragmentManager, customConfig, ::handlePickerResult)
```
...
*   **`PickerResult.Error(error: Throwable)`**: An error occurred during the process (e.g., permission denied, file I/O error).
*   **`PickerResult.Cancelled`**: The user cancelled the operation (e.g., pressed back, tapped outside the dialog).
    ...
    when (result) {
        is PickerResult.Success -> {
            if (result.files.isNotEmpty()) {
                ...
                // User completed the flow but selected nothing
...
*   **Camera:** If you allow the `Camera` source via `PickerConfig` (`allowCamera = true` and image types are permitted), the library will request the `android.permission.CAMERA` permission at runtime if needed. Ensure you declare it in your app's `AndroidManifest.xml` for best practices.
...
The Bottom Sheet dialog uses standard Material Components theming. You can influence its appearance by customizing Material themes (`colorSurface`, `colorOnSurface`, etc.) in your app.
